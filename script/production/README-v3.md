# Polygon ManagedOO V3 production upgrade

This runbook upgrades the existing Polygon `ManagedOptimisticOracleV2` proxy to the separately named
`ManagedOptimisticOracleV3` implementation without reinitializing it. The V2 source, artifacts, scripts, and runbook
remain available unchanged for existing consumers.
...
`old-builds/build-info-prod-current`; the scripts separately pin the raw live runtime code hash and the reviewed V3
target bytecode.
...
NODE_URL_137="$POLYGON_RPC_URL" forge test \
  --match-contract ManagedOptimisticOracleV3ProductionUpgradeForkTest -vv
```
...
DEPLOYER_ADDRESS="$DEPLOYER" forge script \
  script/production/UpgradeManagedOptimisticOracleV3Production.s.sol:\
DeployManagedOptimisticOracleV3ProductionImplementation \
...
TARGET_IMPLEMENTATION="$TARGET" forge script \
  script/production/UpgradeManagedOptimisticOracleV3Production.s.sol:\
PrepareManagedOptimisticOracleV3ProductionSafeUpgrade \
  --rpc-url "$POLYGON_RPC_URL"
```
...
forge script \
  script/production/UpgradeManagedOptimisticOracleV3Production.s.sol:\
VerifyManagedOptimisticOracleV3ProductionUpgrade \
  --rpc-url "$POLYGON_RPC_URL"
```
