...
abstract contract ManagedOptimisticOracleV3ProductionUpgradeChecks {
    ...
    bytes32 internal constant TARGET_CREATION_CODEHASH =
        0x080363b33ee180c2c0530c13d91becadc6e3f8f1cd093fef49701452451ac138;
    bytes32 internal constant TARGET_NORMALIZED_RUNTIME_CODEHASH =
        0x7e6f5cf959545ccfa0fbdbcc3e14f19ca2669c3febad1b68f1a72be893591349;
    ...
    function _assertProductionState(
        ...
        ManagedOptimisticOracleV3 managedOO = ManagedOptimisticOracleV3(PROXY);
    ...
    function _assertTargetImplementation(address implementation) internal view {
        ...
        require(
            ManagedOptimisticOracleV3(implementation).proxiableUUID() == ERC1967_IMPLEMENTATION_SLOT,
            "invalid UUPS UUID"
        );
        require(
    ...
    function _assertRoles(ManagedOptimisticOracleV3 managedOO, address expectedRequestManager) private view {
    ...
    function _adminStateHash(ManagedOptimisticOracleV3 managedOO) private view returns (bytes32) {
        (address pendingAdmin, uint48 pendingSchedule) = managedOO.pendingDefaultAdmin();
        (uint48 pendingDelay, uint48 pendingDelaySchedule) = managedOO.pendingDefaultAdminDelay();
        return keccak256(
            abi.encode(
    ...
    function _configurationStateHash(ManagedOptimisticOracleV3 managedOO) private view returns (bytes32) {
        (uint128 minimumBond, uint128 maximumBond) = managedOO.allowedBondRanges(USDC_E);
        return keccak256(
            abi.encode(
    ...
    function _roleStateHash(ManagedOptimisticOracleV3 managedOO, address expectedRequestManager)
        private
        view
        returns (bytes32)
    {
